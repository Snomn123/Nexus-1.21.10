...
import nexus.mixin.HandledScreenAccessor;
import nexus.mixin.PlayerListHudAccessor;
...
import static nexus.Main.*;

public class Utils {
    public static final MessageIndicator NexusIndicator = new MessageIndicator(0x5ca0bf, null, Text.of("Message from Nexus mod."), "Nexus Mod");
    ...
    public static void showTitleCustom(String title, int stayTicks, int yOffset, float scale, RenderColor color) {
        ((TitleRendering) mc.inGameHud).nexus_mod$setRenderTitle(title, stayTicks, yOffset, scale, color);
    }

    public static boolean isRenderingCustomTitle() {
        return ((TitleRendering) mc.inGameHud).nexus_mod$isRenderingTitle();
    }
    ...
    public static MutableText getTag() {
        return Text.literal("[Nexus] ").withColor(0x5ca0bf);
    }
    ...
    public static void infoRaw(MutableText message) {
        ...
        mc.inGameHud.getChatHud().addMessage(getTag().append(message), null, NexusIndicator);
    }
    ...
    public static void setGlowing(Entity ent, boolean shouldGlow, RenderColor color) {
        ((EntityRendering) ent).nexus_mod$setGlowingColored(shouldGlow, color);
    }
    ...
    public static boolean isGlowing(Entity ent) {
        return ((EntityRendering) ent).nexus_mod$getGlowing();
    }
